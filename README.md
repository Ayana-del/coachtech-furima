...
php artisan db:seed
```  
シーダー実行後、トップページ（/）にアクセスすると、機能要件に基づいた以下の状態を確認できます。  
・商品一覧（おすすめ）：他のユーザーが出品した商品がカード形式で表示されます。  
・Sold表示：ItemsTable Seeder内で販売済みとして設定された商品には「Sold」ラベルが表示されます。  
・フィルタリング：現在ログインしているユーザーが作成した商品は、一覧から自動的に除外されます。  
・マイリスト：ログインユーザーが「いいね」済みの設定で投入された商品は、マイリストタブで確認可能です。  
...
・状態の保持:  
 ...
 ・リンクパラメータにキーワードを含めることで、シームレスなUI/UXを実現しています。  
